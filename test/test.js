...
var iterDigamma = require( './../lib' );


// VARIABLES //

var rand = randu( 0.01, 5.0 );
...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterDigamma, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterDigamma( uniform( 0.01, 5.0 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterDigamma( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the digamma function for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the digamma function for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': digamma( values[ 0 ] ),
			'done': false
		},
		{
			'value': digamma( values[ 1 ] ),
			'done': false
		},
		{
			'value': digamma( values[ 2 ] ),
			'done': false
		},
		{
			'value': digamma( values[ 3 ] ),
			'done': true
		}
	];
